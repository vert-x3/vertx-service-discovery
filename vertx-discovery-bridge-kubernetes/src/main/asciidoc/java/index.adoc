...
Kubernetes services are mapped to `link:../../apidocs/io/vertx/ext/discovery/Record.html[Record]`. This bridge only
supports the importation of services from kubernetes in vert.x (and not the opposite).

`link:../../apidocs/io/vertx/ext/discovery/Record.html[Record]` are created from Kubernetes Service. The service type is deduced from the `service.type` label. If
...
----
compile 'io.vertx:vertx-discovery-bridge-kubernetes:3.3.0-SNAPSHOT'
----
