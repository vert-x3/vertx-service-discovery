...
be populated with `link:unavailable[DiscoveryBridge]`s.
...
You can provide your own bridge by implementing the `link:unavailable[DiscoveryBridge]` interface and
register it using
...
`link:unavailable#stop-instance_method[stop]`
