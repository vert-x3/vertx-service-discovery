...
Kubernetes services are mapped to `link:../../null/dataobjects.html#Record[Record]`. This bridge only
supports the importation of services from kubernetes in vert.x (and not the opposite).

`link:../../null/dataobjects.html#Record[Record]` are created from Kubernetes Service. The service type is deduced from the `service.type` label. If
...
----
compile 'io.vertx:vertx-service-discovery-bridge-kubernetes:3.4.2-SNAPSHOT'
----
