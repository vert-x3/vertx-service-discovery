...
Providers and consumers must create their own `link:../../jsdoc/service_discovery-ServiceDiscovery.html[ServiceDiscovery]` instance. These
...
`link:../../jsdoc/service_reference-ServiceReference.html[ServiceReference]`, representing the binding between the consumer and the provider.
...
`link:../../jsdoc/service_discovery-ServiceDiscovery.html#getReference[getReference]` or released with
`link:../../jsdoc/service_reference-ServiceReference.html#release[release]`, events are emitted on the event bus to track the
service usages.
...
be populated with `link:../../jsdoc/service_discovery_bridge-ServiceDiscoveryBridge.html[ServiceDiscoveryBridge]`s.

== Creating a service discovery instance

Publishers and consumers must create their own `link:../../jsdoc/service_discovery-ServiceDiscovery.html[ServiceDiscovery]`
...
`link:../../jsdoc/service_discovery-ServiceDiscovery.html#getRecords[getRecords]`.
...
* `link:../../jsdoc/jdbc_data_source-JDBCDataSource.html[JDBCDataSource]` - for JDBC data sources, the service object is a
`link:../../jsdoc/jdbc_client-JDBCClient.html[JDBCClient]` (the configuration of the client is computed from the location, metadata and
...
objects are `link:../../jsdoc/http_client-HttpClient.html[HttpClient]` configured with the host, port and ssl.
...
`link:../../jsdoc/http_endpoint-HttpEndpoint.html#createRecord[HttpEndpoint.createRecord]`.

The next snippet illustrates hot to create `link:../dataobjects.html#Record[Record]` from
`link:../../jsdoc/http_endpoint-HttpEndpoint.html[HttpEndpoint]`:
...
`link:../../jsdoc/http_client-HttpClient.html[HttpClient]` with a port and host configured:
...
`link:../../jsdoc/http_endpoint-HttpEndpoint.html#getClient[HttpEndpoint.getClient]`
...
`link:../../jsdoc/service_discovery-ServiceDiscovery.html#releaseServiceObject[ServiceDiscovery.releaseServiceObject]`,
...
methods from `link:../../jsdoc/event_bus_service-EventBusService.html[EventBusService]`.
...
`link:../../jsdoc/message_consumer-MessageConsumer.html[MessageConsumer]`.
...
1. create a record, using `link:../../jsdoc/message_source-MessageSource.html[MessageSource]`
...
`link:../../jsdoc/message_source-MessageSource.html[MessageSource]` class to retrieve the service is one call.
...
When, using `link:../../jsdoc/message_source-MessageSource.html[MessageSource]`, it becomes:
...
using a JDBC driver. The client of a JDBC data source service is a `link:../../jsdoc/jdbc_client-JDBCClient.html[JDBCClient]`.

=== Publishing a JDBC service

As for the other service types, publishing a message source is a 2-steps process:

1. create a record, using `link:../../jsdoc/jdbc_data_source-JDBCDataSource.html[JDBCDataSource]`
2. publish the record
...
`link:../../jsdoc/jdbc_client-JDBCClient.html[JDBCClient]`, are merged: the record location, the metadata and a json object provided by
the consumer:
...
You can also use the `link:../../jsdoc/jdbc_client-JDBCClient.html[JDBCClient]` class to to the lookup and retrieval in one call:
...
You can provide your own bridge by implementing the `link:../../jsdoc/service_discovery_bridge-ServiceDiscoveryBridge.html[ServiceDiscoveryBridge]` interface and
register it using
`link:../../jsdoc/service_discovery-ServiceDiscovery.html#registerDiscoveryBridge[registerDiscoveryBridge]`.
...
exported the initial services, it must complete the given `link:../../jsdoc/future-Future.html[Future]`. If the bridge starts
method is blocking, it must uses an
`link:../../jsdoc/vertx-Vertx.html#executeBlocking[executeBlocking]` construct, and
complete the given future object.

When the service discovery is stopped, the bridge is stopped. The
`link:../../jsdoc/service_discovery_bridge-ServiceDiscoveryBridge.html#stop[stop]`
method is called that provides the opportunity to cleanup resources, removed imported / exported services... This
method must complete the given `link:../../jsdoc/future-Future.html[Future]` to notify the caller of the completion.
