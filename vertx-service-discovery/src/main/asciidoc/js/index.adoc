...
`link:../../jsdoc/discovery_service-DiscoveryService.html#getReference[getReference]` or released with
...
be populated with `link:../../jsdoc/discovery_bridge-DiscoveryBridge.html[DiscoveryBridge]`s.

== Creating the discovery service

Publishers and consumers must create their own `link:../../jsdoc/discovery_service-DiscoveryService.html[DiscoveryService]`
...
service.publish(record, function (ar, ar_err) {
  if (ar_err == null) {
...
service.getRecord(null, function (ar, ar_err) {
  if (ar_err == null) {
    if (ar !== null) {
      // we have a record
    } else {
...
`link:../../jsdoc/discovery_service-DiscoveryService.html#getRecords[getRecords]`.
...
`link:../../jsdoc/discovery_service-DiscoveryService.html#releaseServiceObject[DiscoveryService.releaseServiceObject]`,
...
`link:../dataobjects.html#DiscoveryOptions#setUsageAddress[usageAddress]`.
...
You can provide your own bridge by implementing the `link:../../jsdoc/discovery_bridge-DiscoveryBridge.html[DiscoveryBridge]` interface and
register it using
...
`link:../../jsdoc/discovery_bridge-DiscoveryBridge.html#stop[stop]`
