...
// For http endpoint:
var client = reference1.getService(HttpClient.class);
// For message source
var consumer = reference2.getService(MessageConsumer.class);
...
// Then, gets the service object, the returned type depends on the service type:
// For http endpoint:
var client = reference.getService(JDBCClient.class);
...
`link:../../jsdoc/module-vertx-service-discovery-js_service_reference-ServiceReference.html#getService[getService]`. The parameter is the type of
...
discovery.getRecord({
  ...
  if (ar_err == null && ar !== null) {
    ...
    var client = reference.getService(HttpClient.class);
...
discovery.getRecord({
  ...
  if (ar_err == null && ar !== null) {
    ...
    var reference = discovery.getReference(ar);
    // Retrieve the service object
    var consumer = reference.getService(MessageConsumer.class);
...
discovery.getRecord({
  ...
  if (ar_err == null && ar !== null) {
    ...
    var client = reference.getService(RedisClient.class);
