...
// For http endpoint:
var client = reference1.getAs<Any>(HttpClient.`class`)
// For message source
var consumer = reference2.getAs<Any>(MessageConsumer.`class`)
...
// Then, gets the service object, the returned type depends on the service type:
// For http endpoint:
var client = reference.getAs<Any>(JDBCClient.`class`)
...
discovery.getRecord(json {
  ...
  if (ar.succeeded() && ar.result() != null) {
    ...
    var client = reference.getAs<Any>(HttpClient.`class`)
...
discovery.getRecord(json {
  ...
  if (ar.succeeded() && ar.result() != null) {
    ...
    var reference = discovery.getReference(ar.result())
    // Retrieve the service object
    var client = reference.getAs<Any>(WebClient.`class`)
...
discovery.getRecord(json {
  ...
  if (ar.succeeded() && ar.result() != null) {
    ...
    var reference = discovery.getReference(ar.result())
    // Retrieve the service object
    var consumer = reference.getAs<Any>(MessageConsumer.`class`)
...
MessageSource.getConsumer<JsonObject>(discovery, json {
  obj("name" to "some-message-source-service")
}, { ar ->
  if (ar.succeeded()) {
...
discovery.getRecord(json {
  ...
  if (ar.succeeded() && ar.result() != null) {
    ...
    var client = reference.getAs<Any>(RedisClient.`class`)
