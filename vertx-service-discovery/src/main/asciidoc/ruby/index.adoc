...
# For http endpoint:
client = reference1.get_service(Vertx::HttpClient::class)
# For message source
consumer = reference2.get_service(Vertx::MessageConsumer::class)
...
# Then, gets the service object, the returned type depends on the service type:
# For http endpoint:
client = reference.get_service(VertxJdbc::JDBCClient::class)
...
`link:../../yardoc/VertxServiceDiscovery/ServiceReference.html#get_service-instance_method[getService]`. The parameter is the type of
...
discovery.get_record({
  ...
  if (ar_err == nil && ar != nil)
    ...
    client = reference.get_service(Vertx::HttpClient::class)
...
discovery.get_record({
  ...
  if (ar_err == nil && ar != nil)
    ...
    reference = discovery.get_reference(ar)
    # Retrieve the service object
    consumer = reference.get_service(Vertx::MessageConsumer::class)
...
discovery.get_record({
  ...
  if (ar_err == nil && ar != nil)
    ...
    client = reference.get_service(VertxRedis::RedisClient::class)
