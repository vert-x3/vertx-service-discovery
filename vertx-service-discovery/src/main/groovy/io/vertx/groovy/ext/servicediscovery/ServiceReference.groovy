...
/**
 * Once a consumer has chosen a service, it builds a {@link io.vertx.groovy.ext.servicediscovery.ServiceReference} managing the binding with the chosen
 * service provider.
 * <p>
 * The reference lets the consumer:
 * * access the service (via a proxy or a client) with the {@link io.vertx.groovy.ext.servicediscovery.ServiceReference#get} method
 * * release the reference - so the binding between the consumer and the provider is removed
*/
@CompileStatic
public class ServiceReference {
  private final def io.vertx.ext.servicediscovery.ServiceReference delegate;
  public ServiceReference(Object delegate) {
    this.delegate = (io.vertx.ext.servicediscovery.ServiceReference) delegate;
  }
